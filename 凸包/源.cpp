...
// direction 函数确定了线段 p1p 与 p2p 相对于点 p 的方向
// 如果返回值为正，则表示顺时针方向；
// 如果返回值为负，则表示逆时针方向；
// 如果返回值为零，则表示在同一条直线上
...
//function on_segment determines whether the point p is on the segment p1p2函数确定了点 p 是否在线段 p1p2 上
...
//point startPoint is the polor point that is needed for comparing two other points;是用于比较两个其他点的极点
POINT startPoint;
//function sortByPolorAngle provides the realizing of comparing two points, which support

//the STL function sort(); 函数实现了比较两个点的过程，支持 STL 函数 sort()
...
//here realizes the process of finding convex hull // 实现了查找凸包的过程
...
//这个函数创建了一个 POINT 对象的向量，
//表示平面上的点。然后调用 find_convex_hull 函数，
//并将 points 作为其参数。结果的凸包会打印到控制台上。